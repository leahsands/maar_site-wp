...
function wplc_pro_admin_display_history() {
    ...
    echo "
       <form method=\"post\" >
        <input type=\"submit\" value=\"Delete History\" class='button' name=\"wplc-delete-chat-history\" /><br /><br />
       </form>
    ...
    if (!$results) {
        ...
        foreach ($results as $result) {
            ...
            $actions = "<a href='$url' class='button' title='View Chat History' target='_BLANK' id=''>View Chat History</a>";
...
function wplc_pro_output_box() {
    ...
    if ($wplc_settings["wplc_settings_fill"]) { $wplc_settings_fill = "#".$wplc_settings["wplc_settings_fill"]; } else {  $wplc_settings_fill = "#ec832d"; }
    ...
    if ($wplc_is_admin_logged_in == 1 or $wplc_is_admin_logged_in == true) {
        $wplc_tl_msg = "<strong style=\"color: ".$wplc_settings_font." !important;\">".stripslashes($wplc_pro_settings['wplc_pro_fst1'])."</strong> ".stripslashes($wplc_pro_settings['wplc_pro_fst2']);    } else {
...
?>    
        ...
        <div id="wp-live-chat-2" style="display:none;">
            ...
            <div id="wp-live-chat-2-info">
               <?php echo stripslashes($wplc_pro_settings['wplc_pro_intro']); ?> 
            </div>
                ...
                <input id="wplc_start_chat_btn" type="button" value="<?php echo $wplc_pro_settings['wplc_pro_sst1'] ?>" style="background-color: <?php echo $wplc_settings_fill; ?> !important; color: <?php echo $wplc_settings_font; ?> !important;"/>
            ...
            ?>
            <div id="wp-live-chat-2-info">
                 <?php echo stripslashes($wplc_pro_settings['wplc_pro_offline1']); ?>
            </div>
        ...
        <div id="wp-live-chat-4" style="display:none;">
            ...
            <div id="wplc_chatbox">
                <span class='wplc-admin-message'>
                    <?php if($wplc_pro_settings['wplc_chat_pic']){ ?>
                        <img src="<?php echo urldecode($wplc_pro_settings['wplc_chat_pic']); ?>" width="20px" id="wp-live-chat-2-img"/>
                    <?php } ?>
                    <strong><?php echo stripslashes($wplc_pro_settings['wplc_chat_name']); ?>:</strong>
                    <hr/> 
                    <?php echo stripslashes($wplc_pro_settings['wplc_user_welcome_chat']); ?>
                </span>
                <br />
...
function wplc_head_pro() {
    ...
    if (isset($_POST['wplc_save_settings'])){
        ...
        $wplc_pro_data = array();
        if (isset($_POST['wplc_pro_name'])) { $wplc_pro_data['wplc_chat_name'] = esc_attr($_POST['wplc_pro_name']); }
        if (isset($_POST['wplc_upload_pic'])) { $wplc_pro_data['wplc_chat_pic'] = esc_attr(urlencode(base64_decode($_POST['wplc_upload_pic']))); }
        if (isset($_POST['wplc_upload_logo'])) { $wplc_pro_data['wplc_chat_logo'] = esc_attr(urlencode(base64_decode($_POST['wplc_upload_logo']))); }
        if (isset($_POST['wplc_pro_delay'])) { $wplc_pro_data['wplc_chat_delay'] = esc_attr($_POST['wplc_pro_delay']); }
        if (isset($_POST['wplc_pro_chat_notification'])) { $wplc_pro_data['wplc_pro_chat_notification'] = esc_attr($_POST['wplc_pro_chat_notification']); }

        if (isset($_POST['wplc_pro_na'])) { $wplc_pro_data['wplc_pro_na'] = esc_attr($_POST['wplc_pro_na']); }
        if (isset($_POST['wplc_pro_chat_email_address'])) { $wplc_pro_data['wplc_pro_chat_email_address'] = esc_attr($_POST['wplc_pro_chat_email_address']); }
        if (isset($_POST['wplc_pro_fst1'])) { $wplc_pro_data['wplc_pro_fst1'] = esc_attr($_POST['wplc_pro_fst1']); }
        if (isset($_POST['wplc_pro_fst2'])) { $wplc_pro_data['wplc_pro_fst2'] = esc_attr($_POST['wplc_pro_fst2']); }
        if (isset($_POST['wplc_pro_fst3'])) { $wplc_pro_data['wplc_pro_fst3'] = esc_attr($_POST['wplc_pro_fst3']); }
        if (isset($_POST['wplc_pro_sst1'])) { $wplc_pro_data['wplc_pro_sst1'] = esc_attr($_POST['wplc_pro_sst1']); }
        if (isset($_POST['wplc_pro_sst2'])) { $wplc_pro_data['wplc_pro_sst2'] = esc_attr($_POST['wplc_pro_sst2']); }
        if (isset($_POST['wplc_pro_tst1'])) {  $wplc_pro_data['wplc_pro_tst1'] = esc_attr($_POST['wplc_pro_tst1']); }

        if (isset($_POST['wplc_pro_offline1'])) { $wplc_pro_data['wplc_pro_offline1'] = esc_attr($_POST['wplc_pro_offline1']); }
        if (isset($_POST['wplc_pro_offline2'])) { $wplc_pro_data['wplc_pro_offline2'] = esc_attr($_POST['wplc_pro_offline2']); }
        if (isset($_POST['wplc_pro_offline3'])) { $wplc_pro_data['wplc_pro_offline3'] = esc_attr($_POST['wplc_pro_offline3']); }

        if (isset($_POST['wplc_pro_intro'])) { $wplc_pro_data['wplc_pro_intro'] = esc_attr($_POST['wplc_pro_intro']); }

        if (isset($_POST['wplc_user_enter'])) { $wplc_pro_data['wplc_user_enter'] = esc_attr($_POST['wplc_user_enter']); }
        if (isset($_POST['wplc_user_welcome_chat'])) { $wplc_pro_data['wplc_user_welcome_chat'] = esc_attr($_POST['wplc_user_welcome_chat']); }
...
function wplc_pro_admin_menu_layout_display() {
   ...
   if (!isset($_GET['action'])) {
        ...
        <?php //if(function_exists("wplc_ma_register")){
...
function wplc_pro_draw_chat_area($cid) {
    ...
    foreach ($results as $result) {
        ...
        echo "<div id='wplc_admin_chat'>";
            ...
            if ($result->status == 3) {
            echo "
                    <p style=\"text-align:left; font-size:11px;\">".__("Press ENTER to send your message","wplivechat")."</p>
        ...
        function wplc_call_to_server_admin_chat(data) {
			...
			wplc_run = false;
                ...
                complete: function(response){
                    ...
                    if (wplc_run) { 
                        setTimeout(wplc_call_to_server_admin_chat(data), 1500); 
                    }
                },
                timeout: 120000
            });
                        ...
                        if(response['action'] === "wplc_update_admin_chat"){
                            ...
                            if (response['pending'] === true) {
                                ...
                                wplc_pending_refresh = setInterval(function (){
                                    ...
                                    wplc_title_alerts2 = setTimeout(function (){ document.title = orig_title; }, 2000);
                                    wplc_title_alerts4 = setTimeout(function (){ document.title = "** CHAT REQUEST **"; }, 4000);
		...
		error: function(jqXHR, exception) {
			...
			wplc_run = false;
                ...
                complete: function(response){
                    //console.log(wplc_run);
                    if (wplc_run) { 
                        setTimeout(wplc_call_to_server(data), 3000);
                    }
                },
                timeout: 120000
            });
...
$wplc_api_url = 'http://wp-livechat.com/apid/';
